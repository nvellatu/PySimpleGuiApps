...
def test_deal_hand():
    ...
    if not type(hand) is dict:
        print
        "FAILURE: test_deal_hand()"
        print
        "\tUnexpected return type:", type(hand)
    ...
    for k in hand.keys():
        if (not type(k) is str) or (not type(hand[k]) is int):
            ...
            "FAILURE: test_deal_hand()"
            print
            "\tUnexpected type of dictionary: string -> int expected, but was", type(k), "->", type(hand[k])
            ...
            "FAILURE: test_deal_hand()"
            print
            "\tDictionary keys are not lowercase letters."
    ...
    if num != HAND_SIZE:
        ...
        "FAILURE: test_deal_hand()"
        print
        "\tdeal_hand() returned more letters than it was asked to."
        print
        "\tAsked for a hand of size", HAND_SIZE, "but it returned a hand of size", num
    ...
    if repeats > 10:
        ...
        "FAILURE: test_deal_hand()"
        print
        "\tSame hand returned", repeats, "times by deal_hand(). This is HIGHLY unlikely."
        print
        "\tIs the deal_hand implementation really using random numbers?"

        return  # exit function

    print
    "SUCCESS: test_deal_hand()"
...
def test_get_word_score():
    ...
    for (word, n) in words.keys():
        ...
        if score != words[(word, n)]:
            ...
            print
            "\tExpected", words[(word, n)], "points but got '" + str(score) + "' for word '" + word + "', n=" + str(n)
            failure = True
    if not failure:
        print
...
def test_update_hand():
    ...
    if hand2 != expected_hand1 and hand2 != expected_hand2:
        print
        "FAILURE: test_update_hand('" + word + "', " + str(hand) + ")"
        print
        "\tReturned: ", hand2, "-- but expected:", expected_hand1, "or", expected_hand2
...
def test_is_valid_word(word_list):
    ...
    if not is_valid_word(word, hand, word_list):
        ...
        print
        "\tExpected True, but got False for word: '" + word + "' and hand:", hand
    ...
    if is_valid_word(word, hand, word_list):
        ...
        "FAILURE: test_is_valid_word()"
        print
        "\tExpected False, but got True for word: '" + word + "' and hand:", hand
